...
-- Rule R8: If something chases the tiger then it visits the bald eagle
axiom R8 : ∀ x : obj, Chases x Tiger → Visits x BaldEagle
-- Rule R9: If something chases the rabbit and it does not visit the lion then the lion does not need the rabbit
axiom R9 : ∀ x : obj, Chases x Rabbit ∧ ¬ Visits x Lion → ¬ Needs Lion Rabbit


theorem not_chase_bald_eagle : ¬ Chases BaldEagle BaldEagle :=
begin
  sorry
end


theorem chase_bald_eagle : Chases BaldEagle BaldEagle :=
begin
  sorry
end
